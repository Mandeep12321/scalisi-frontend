...
// Optional: wrapper for each carousel item
const ItemWrapper = ({ itemPosition, children }) => {
  return (
    <div className={classes.itemWrapper} data-item-position={itemPosition}>
      {children}
    </div>
  );
};
    ...
    `}
      ...
      <div className={classes.carouselWrapper}>
        <Carousel breakPoints={breakPoints} pagination={true}>
          {data?.map((item, index) => (
            <ItemWrapper key={index} itemPosition={index}>
              <HomeHeroSec item={item} />
            </ItemWrapper>
          ))}
        </Carousel>
      </div>
